// ZENO — "Extend My Stay" Feature: Success Landing Page
// Route: /payment/extend-success?session_id=cs_test_...
// Verifies the Stripe session server-side, then confirms the extension.
...
export default function ExtendSuccess() {
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        const res = await API.post("/payments/verify-extend-session", { sessionId });
        if (res.data.success) {
        ...
        {state === "success" && (
          <>
            ...
            <p className="mt-4 font-mono text-sm text-gray-600">
              {booking?.endTime
                ? `Your booking now runs until ${new Date(booking.endTime).toLocaleString()}.`
